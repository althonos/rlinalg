...
def setup(app):
    ...
    def inspector(app, what, name, obj, options, lines):
        if obj.__class__.__name__ == "fortran":
            lines.pop(0)

    app.connect("autodoc-process-docstring", inspector)
...
project = "Rlinalg"
author = re.match("(.*) <.*>", rlinalg.__author__).group(1)
year = datetime.date.today().year
copyright = "{}, {}".format("2024" if year == 2024 else "2024-{}".format(year), author)

# extract the semantic version
semver = semantic_version.Version.coerce(rlinalg.__version__)
...
doc_lines = rlinalg.__doc__.splitlines()
...
#
html_theme_options = {
    "github_url": "https://github.com/althonos/rlinalg",
    "twitter_url": "https://twitter.com/althonos",
